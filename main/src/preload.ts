...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  sessions: {
    ...
    // 支持可选参数控制是否删除本地/远程分支
    delete: (sessionId: string, options?: { deleteLocalBranch?: boolean; deleteRemoteBranch?: boolean }): Promise<IPCResponse> => ipcRenderer.invoke('sessions:delete', sessionId, options),
    deletePermanent: (sessionId: string, options?: { deleteLocalBranch?: boolean; deleteRemoteBranch?: boolean }): Promise<IPCResponse> => ipcRenderer.invoke('sessions:delete-permanent', sessionId, options),
    ...
    smartCommit: (sessionId: string, options?: { messageOverride?: string; addAll?: boolean }): Promise<IPCResponse> => ipcRenderer.invoke('sessions:smart-commit', sessionId, options),
  ...
  events: {
    ...
    onSessionArchived: (callback: (session: Session) => void) => {
      const wrappedCallback = (_event: Electron.IpcRendererEvent, session: Session) => callback(session);
    ...
    onPanelActiveChanged: (callback: (data: { sessionId: string; panelId: string | null }) => void) => {
      const wrappedCallback = (_event: Electron.IpcRendererEvent, data: { sessionId: string; panelId: string | null }) => callback(data);
